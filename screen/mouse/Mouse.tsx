...
export default class Mouse extends Component<
  MousePropsComponent,
  { pressure: string; dx: number; dy: number }
> {
  private dx: number = 0;
  private dy: number = 0;
  ...
  moveCallback(dx: number, dy: number) {
    _moveCallback(dx, dy, Number(this.state.pressure)).then((mouse) =>
      this.updateCoordinates(mouse)
    );
  }
    ...
    return (
      <View style={styles.container}>
        <View style={styles.padContainer}>
          <Pad
            moveCallback={(dx: number, dy: number) => this.moveCallback(dx, dy)}
        ...
        <View style={styles.coordinates}>
          <TextInputIcon
            value={String(this.dx)}
            icon={"axis-x-arrow"}
            size={30}
            color={"#517fa4"}
          />
          <TextInputIcon
            value={String(this.dy)}
            icon={"axis-y-arrow"}
            size={30}
            color={"#517fa4"}
          />
